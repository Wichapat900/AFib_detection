"""
preprocess.py — AFib_Detection Data Preprocessing
================================================
...
EXCLUDE = {"00735", "03665", "08405", "08434", "08465", "08475", "05091"}
BAD_BLOCKS   = {"04043"} #Block 39 gone, fill in the "hole"(Just a couple queations)
...
def preprocess():
    ...
    summary = {
        ...
        "shape":            list(X.shape),
        "excluded_records": list(EXCLUDE),
        "bad_blocks":       list(BAD_BLOCKS),
        "timestamp":        __import__('datetime').datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }
    with open(OUT_PATH / "preprocess_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"Summary saved → data/processed/preprocess_summary.json")
